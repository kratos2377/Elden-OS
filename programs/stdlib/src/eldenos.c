...
struct command_argument* eldenos_parse_command(const char* command, int max)
{
    ...
    root_command = eldenos_malloc(sizeof(struct command_argument));
    if (!root_command)
    {
        goto out;
    }
    ...
    while(token != 0)
    {
        struct command_argument* new_command = eldenos_malloc(sizeof(struct command_argument));
        if (!new_command)
        {
            break;
        }
...
int eldenos_system_run(const char* command)
{
    ...
    struct command_argument* root_command_argument = eldenos_parse_command(buf, sizeof(buf));
    if (!root_command_argument)
    {
        return -1;
    }

    return eldenos_system(root_command_argument);
}