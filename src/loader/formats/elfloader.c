...
{
    return header->e_type == ET_EXEC && header->e_entry >= ELDENOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
{
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? ELDENOS_ALL_OK : -EINFORMAT;
}
